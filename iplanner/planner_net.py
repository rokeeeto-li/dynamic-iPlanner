...
class PlannerNet(nn.Module):
    ...
    def forward(self, x, goal, vel):
        x = self.encoder(x)
        x, c = self.decoder(x, goal, vel)
        return x, c


class Decoder(nn.Module):
    def __init__(self, in_channels, goal_channels, vel_channels=5, k=5):
        super().__init__()
        self.k = k # waypoints amount
        self.relu    = nn.ReLU(inplace=True)
        self.fg      = nn.Linear(3, goal_channels)
        self.fv      = nn.Linear(3, vel_channels)
        self.sigmoid = nn.Sigmoid()

        self.conv1 = nn.Conv2d((in_channels + goal_channels + vel_channels), 512, kernel_size=5, stride=1, padding=1)
    ...
    def forward(self, x, goal, vel):
        ...
        if torch.any(torch.isnan(vel)):
            print("/n The nan index is", torch.where(torch.isnan(vel)))
            print("vel has nan")

        vel  = self.fv(vel[:, 0:3])
        vel  = vel[:, :, None, None].expand(-1, -1, x.shape[2], x.shape[3])
